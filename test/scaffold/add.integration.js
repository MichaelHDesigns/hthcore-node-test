...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/hthcore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update hthcore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          '@dashevo/hthcore-lib': '^v0.15.4',
          '@dashevo/hthcore-node': '^v3.0.7'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/hthcore-node.json');

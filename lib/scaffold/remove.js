...
/**
 * Will remove the Node.js service and from the hthcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The hthcore-node configuration directory
...
function remove(options, done) {
  ...
  var dashcoreConfigPath = path.resolve(configPath, 'hthcore-node.json');
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to hthcore-node.json
